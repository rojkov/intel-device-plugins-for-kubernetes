...
func concatStr(values ...string) string {
...
func getDpdkDevice(id string) (string, error) {
	...
	case "vfio-pci":
		vfioDirPath := concatStr(pciDeviceDir, devicePCIAdd, iommuGroupSuffix)
...
func getDpdkDeviceNames(id string) ([]string, error) {
	...
	case "", "igb_uio":
		//Setting up with uio
		uioDev := concatStr(uioDevicePath, dpdkDeviceName)
		return []string{uioDev}, nil
	case "vfio-pci":
		//Setting up with vfio\n
		vfioDev1 := concatStr(vfioDevicePath, dpdkDeviceName)
		vfioDev2 := concatStr(vfioDevicePath, "/vfio")
...
func getDpdkMountPaths(id string) ([]string, error) {
	...
	case "", "igb_uio":
		...
		uioMountPoint := concatStr(uioMountPath, dpdkDeviceName, "/device")
...
func (dm *deviceManager) discoverQATs() (bool, error) {
	...
	for _, kernelvfDriver := range kernelvfDrivers {
		...
		if err != nil {
			return found, fmt.Errorf("Error in user input for DPDK Device Driver: %v", err)
		}
